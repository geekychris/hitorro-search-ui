 ...
 */
export function AutoRenderer({ hit, schema, lang, query, onOpen }: TypeRendererProps) {
  const doc = hit.doc ?? {}
  ...
  return (
    <div
      className="p-4 rounded-lg border border-slate-200 hover:border-hitorro-primary hover:shadow-md cursor-pointer transition bg-white"
      onClick={onOpen}
    >
      {/* header row: title + type chip + first date badge */}
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0">
          <h3 className="text-base font-semibold text-slate-900 truncate">{String(title)}</h3>
      ...
      {identifierChips.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {identifierChips.map((c, i) => <FieldChip key={i} field={c.field} value={c.value} />)}
        </div>
      )}
 ...
 */
function FieldChip({ field, value }: { field: string; value: any }) {
  if (typeof value === 'boolean') {
    // Boolean chips — colour speaks for the state so no value text needed.
    const [label, cls] = booleanChip(field, value)
    return <span className={`px-1.5 py-0.5 text-[11px] rounded ${cls}`}>{label}</span>
  }
  const s = String(value)
  const short = s.length > 40 ? s.slice(0, 40) + '…' : s
  return (
    <span
      className="px-1.5 py-0.5 text-[11px] rounded bg-slate-100 text-slate-700"
      title={`${field}: ${s}`}
    >
      <span className="text-slate-500">{field}:</span> {short}
    </span>
  )
}
